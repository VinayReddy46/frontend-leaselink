...
//   const location = useLocation(); // ✅ Get product data from state
...
//                   className="mt-1  h-[50px] block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
//                 />
//               </div>
//               <div>
...
const ProductDetails = () => { 
  ...
  // Calculate total hours for rental
  const calculateTotalHours = () => {
    if (!startDate || !endDate) return 0;
    const start = new Date(startDate);
    const end = new Date(endDate);
  ...
  const calculateTotal = () => {
    const hours = calculateTotalHours();
    const rentAmount = hours * (product.pricePerHour || 0);
    const insuranceAmount = selectedInsurance ? selectedInsurance.price : 0;
    return rentAmount + insuranceAmount;
  };
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <motion.div>
          ...
          <p className="text-gray-600 mt-2">{product.processor}</p>
          <p className="text-gray-600">{product.brand}</p>
          <p className="text-blue-500 font-bold text-lg mt-2">₹{product.price}</p>
        </motion.div>

        {/* Rental and Insurance Section */}
        <motion.div className="bg-gray-50 p-6 rounded-lg">
          <h3 className="text-xl font-semibold mb-4">Rental Details</h3>
          ...
          {startDate && endDate && (
            <div className="mt-4 text-sm text-gray-600">
              Duration: {calculateTotalHours()} hours
            </div>
          )}
          ...
          <div className="mt-6 border-t pt-6">
            <div className="flex justify-between text-sm mb-2">
              <span>Rental Amount ({calculateTotalHours()} hours)</span>
              <span>₹{calculateTotalHours() * (product.pricePerHour || 0)}</span>
            </div>
            {selectedInsurance && (
              <div className="flex justify-between text-sm mb-2">
                <span>Insurance ({selectedInsurance.name})</span>
